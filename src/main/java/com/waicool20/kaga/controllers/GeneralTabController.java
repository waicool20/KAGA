...
public class GeneralTabController {
    ...
    private void setValues() {
        ...
        paranoiaSpinner.setValueFactory(
            new SpinnerValueFactory.IntegerSpinnerValueFactory(0, Integer.MAX_VALUE));
        sleepCycleSpinner.setValueFactory(
            new SpinnerValueFactory.IntegerSpinnerValueFactory(0, Integer.MAX_VALUE));
        sleepModifierSpinner.setValueFactory(
            new SpinnerValueFactory.IntegerSpinnerValueFactory(0, Integer.MAX_VALUE));
    }

    private void createBindings() {
        recoveryMethodChoiceBox.valueProperty()
            .bindBidirectional(Kaga.PROFILE.getGeneral().recoveryMethodProperty());
        programTextField.textProperty()
            .bindBidirectional(Kaga.PROFILE.getGeneral().programProperty());
        basicRecoveryCheckBox.selectedProperty()
            .bindBidirectional(Kaga.PROFILE.getGeneral().basicRecoveryProperty());
        ObjectBindings.bindBidirectionally(paranoiaSpinner.getValueFactory().valueProperty(),
            Kaga.PROFILE.getGeneral().paranoiaProperty());
        ObjectBindings.bindBidirectionally(sleepCycleSpinner.getValueFactory().valueProperty(),
            Kaga.PROFILE.getGeneral().sleepCycleProperty());
        ObjectBindings.bindBidirectionally(sleepModifierSpinner.getValueFactory().valueProperty(),
            Kaga.PROFILE.getGeneral().sleepModifierProperty());
    }
}
